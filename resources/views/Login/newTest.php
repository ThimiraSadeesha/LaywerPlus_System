...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['submit_otp'])) {
        ...
        // Validate the entered OTP against the stored OTP in the database
        $validate_otp_sql = "SELECT * FROM inactive_users WHERE email = '$email' AND otp = '$enteredOTP' AND otp_expiration < NOW()";
        $result = $conn->query($validate_otp_sql);

        if ($result && $result->num_rows > 0) {
            echo "Welcome, " . $fullName . "! Your account is now active.";
            echo '<a href="login/login.php">Login In</a>';

            $conn->query("UPDATE inactive_users SET status = 'active' WHERE email = '$email'");
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <?php if ($showOtpField) { ?>
                            ...
                            <h4 class="card-title text-center mb-4">Enter Email Verification Code</h4>
                            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                                <div class="mb-3">
